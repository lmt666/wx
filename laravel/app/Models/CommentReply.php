...
class CommentReply extends Model
{
    ...
    // 判断reply和comment是否正确匹配
    public function reply_match_comment($comment_id, $reply_id){
        $data = CommentReply::where('id', $reply_id)->get()->toArray();

        if($comment_id == $data[0]['comment_id']){
            return true;
        }else{
            return false;
        }
